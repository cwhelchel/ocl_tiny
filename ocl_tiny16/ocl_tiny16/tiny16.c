...
test ()
{
#pragma omp declaration id
  int		id;


  omp_init_lock (&lck);
#pragma omp parallel private (id) num_threads( (omp_get_max_threads ()) )
  {
    int	cnt = 0;
...
int main (int argc, char *argv[])
{
  ...
  for (i=thds;  i<N;  i++) {
    if (y[i] != -1) {
      errors += 1;
      printf ("omp_lock 016 - expected y[%d] = %d, observed %d\n",
	      i, -1, y[i]);
    }
  }

  if (errors == 0) {
    printf ("omp_lock 016 PASSED\n");
  } else {
    printf ("omp_lock 016 FAILED\n");
  }
  return 0;
}
